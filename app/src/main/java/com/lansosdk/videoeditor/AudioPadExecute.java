 ...
 *  当前的音频格式支持 MP3,WAV, AAC(m4a后缀), 采样率为44100, 通道数为2,其他格式暂不支持,请注意
 *  当前的音频格式支持 MP3,WAV, AAC(m4a后缀), 采样率为44100, 通道数为2,其他格式暂不支持,请注意
 *  当前的音频格式支持 MP3,WAV, AAC(m4a后缀), 采样率为44100, 通道数为2,其他格式暂不支持,请注意
 *  当前的音频格式支持 MP3,WAV, AAC(m4a后缀), 采样率为44100, 通道数为2,其他格式暂不支持,请注意
 */
public class AudioPadExecute {
	...
	/**
	 * AudioPad容器处理两种情况的声音:
	 * 1, 一整段音乐上增加别的音频, (这里是第一种);
	 * 2,设置容器处理的声音总长度, 然后分别增加不同声音段;
	 * 这里是第一种;
	 * 
	 ...
	 */
	  ...
	  * @param endAudioUs   该音频的结束时间.
	  * @return
	  */
	 public AudioSource addSubAudio(String srcPath,long startPadUs,long startAudioUs,long endAudioUs)
	 {
		 if(audioPad!=null){
			 return audioPad.addSubAudio(srcPath, startPadUs, startAudioUs, endAudioUs);
		 }else{
			 return null;
		 }
	 }
	 ...
	 /**
	  * 等待执行完毕;适合在音频较短,为了代码的整洁, 不想设置listener回调的场合;
	  * 注意:这里设置后, 当前线程将停止在这个方法处,直到音频执行完毕退出为止.建议放到另一个线程中执行.
	  * 可选使用.
	  */
	...
	//----------------------------一下为测试代码------------------------------------------- 
	 ...
	 {
	    	AudioPadExecute   audioPad=new AudioPadExecute(ctx,"/sdcard/i9.m4a");
	    	AudioSource asource=audioPad.setAudioPadSource("/sdcard/audioPadTest/niu30s_44100_2.m4a");
	    	
	    	AudioSource asource2=audioPad.addSubAudio("/sdcard/audioPadTest/hongdou10s_44100_2.mp3", 3*1000*1000,0, 3*1000*1000); //中间3秒增加一段
...
//	 //举例3: 先设置AudioPad的总长度, 然后在不同的时间点增加几段声音.处理成同一个.
	 ...
	 public static void test3(Context ctx)
	 {
		 AudioPadExecute   audioPad=new AudioPadExecute(ctx,"/sdcard/i8.m4a");
	    	...
	    	audioPad.addSubAudio("/sdcard/audioPadTest/du15s_44100_2.mp3", 0,0, 3*1000*1000);  //在这15内, 的前3秒增加一个声音
	    	audioSrc1=audioPad.addSubAudio("/sdcard/audioPadTest/hongdou10s_44100_2.mp3", 3*1000*1000,0, 3*1000*1000); //中间3秒增加一段
	    	audioPad.addSubAudio("/sdcard/audioPadTest/niu30s_44100_2.m4a", 10*1000*1000,0, -1);  //最后3秒增加一段.
  ...
  //举例4:时间拉伸演示
	 public static void test4(Context ctx)
	 {
		 AudioPadExecute   audioPad=new AudioPadExecute(ctx,"/sdcard/i10.m4a");
	    	AudioSource  source=audioPad.setAudioPadSource("/sdcard/audioPadTest/niu30s_44100_2.m4a");
...
//	    	  3,原音频的结束时间:单位微秒;
	    	...
	    	audioPad.addSubAudio("/sdcard/audioPadTest/hongdou10s_44100_2.mp3", 3*1000,0, 6*1000); //中间3秒增加一段
