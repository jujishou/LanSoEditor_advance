...
public class ViewLayerOnlyActivity extends Activity implements IEndListener {
    ...
    private void initDrawPad() {
        ...
        drawPadView.setRealEncodeEnable(480, 480, 1000 * 1000, (int) 25,
                editorTmpPath);
    ...
    private void playAudio() {
        if (audioPlayer == null) {
            ...
            audioPath = CopyFileFromAssets.copyAssets(getApplicationContext(),
                    "thrid50s.m4a");
        ...
        public void onCompleted(DrawPad v) {
            if (isDestorying == false) {
                if (SDKFileUtils.fileExist(editorTmpPath)) {
                    ...
                    audioPath = CopyFileFromAssets.copyAssets(
                            getApplicationContext(), "thrid50s.m4a");
                    VideoEditor editor = new VideoEditor();
                    editor.executeVideoMergeAudio(editorTmpPath, audioPath,
                            dstPath);
                    findViewById(R.id.id_viewLayer_saveplay).setVisibility(
                            View.VISIBLE);
                }
                toastStop();
            }
