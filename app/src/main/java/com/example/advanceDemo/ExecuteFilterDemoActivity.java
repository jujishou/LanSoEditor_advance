 ...
 * <p>
 * <p>
 * 比如你想设计的:在滤镜的过程中,让用户手动拖动增加一些图片,文字等, 增加完成后,记录下用户的操作信息, 但需要统一处理时,通过此类来在后台执行.
 * <p>
 * 流程是:使用DrawPadVideoExecute,
 * 创建一个DrawPad,从中增加VideoLayer,向内部增加各种滤镜,然后设置到视频播放器中,在画面播放过程中,可以随时增加另外的一些
...
public class ExecuteFilterDemoActivity extends Activity {
    ...
    private void startDrawPadExecute() {
        ...
        mDrawPad = new DrawPadVideoExecute(ExecuteFilterDemoActivity.this,
                videoPath, padWidth, padHeight, (int) (mInfo.vBitRate * 1.5f),
    ...
    private void drawPadProgress(DrawPad v, long currentTimeUs) {
        ...
        /**
         * 演示在中间切换滤镜 在3秒的时候, 切换一个滤镜. 在6秒的时候, 再次切换一个滤镜
         */
        if (currentTimeUs > 6 * 1000 * 1000) {
            if (mainVideoLayer != null && isNashvilleFilter == false) {
                mainVideoLayer.switchFilterTo(new IFNashvilleFilter(
                    ...
                    public void onClick(View v) {
                        if (SDKFileUtils.fileExist(dstPath)) {
                            Intent intent = new Intent(
                                    ExecuteFilterDemoActivity.this,
                                    VideoPlayerActivity.class);
                            intent.putExtra("videopath", dstPath);
                            startActivity(intent);
                        } else {
                            Toast.makeText(ExecuteFilterDemoActivity.this,
